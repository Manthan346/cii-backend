...
export const createAssessment = asyncHandler(
    async (req: InstructorAuthRequest, res: Response) => {
        ...
        const trimmedDescription = assessment_desc?.trim() ?? null;
        const trimmedAssessmentLink = assessment_link?.trim() ?? null;
        ...
        if (batch.b_status !== batch_status.ACTIVE) {
            throw new ApiError(400, "Assessment can only be created for active batches.");
        }

        switch (user.user_role) {
            case "instructor":
                if (batch.course_details.company_id !== req.instructor?.company_id) {
                    throw new ApiError(403, "You are not authorized to create assessment for this batch.");
                }
                break;
            ...
            default:
                throw new ApiError(403, "You are not authorized to create assessment.");
        }

        // Fetch enrolled students before beginning the transaction.
        // Single query provides both candidate_id (for cache invalidation)
        // and the related user_id (for notifications) — avoids the duplicate
        // query that was previously run again inside the transaction.
