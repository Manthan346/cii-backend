...
export const getStudyMaterial = asyncHandler(
    async (req: InstructorAuthRequest, res: Response) => {
        ...
        if (!instructor.company_id) {
            throw new ApiError(
                403,
                "Instructor is not associated with any company."
            );
        }
        ...
        const batchStatus = String(
            req.query.batch_status || "ALL"
        );

        if (
            !Object.values(batch_status).includes(
        ...
        const totalRecords =
            await prisma.study_material.count({
                where: {
                    ...studyMaterialFilter,
                    ...searchFilter,
                    batch_details: companyBatchFilter
                }
            });
        ...
        const studyMaterials =
            await prisma.study_material.findMany({
